...
export async function executeRun(
  ...
  try {
    ...
    for (let i = 1; i <= config.maxIterations; i++) {
      ...
      let suggestion: TargetSuggestion | null = null;
...
export async function executeBatch(
  ...
  const runSpecs: Array<{ task: ExperimentTask; condition: ExperimentCondition; index: number }> = [];
  let index = 0;
...
async function executeParallel(
  runSpecs: Array<{ task: ExperimentTask; condition: ExperimentCondition; index: number }>,
  agent: ExperimentAgent,
  ...
  const results: Array<{ run?: ExperimentRun; error?: Error }> = [];
 ...
 */
export async function executeTaskAcrossConditions(
  task: ExperimentTask,
  design: ExperimentDesign,
  agent: ExperimentAgent,
  options: RunOptions = {}
 ...
 */
export async function executeBaselineVsTreatment(
  task: ExperimentTask,
  design: ExperimentDesign,
  agent: ExperimentAgent,
  options: RunOptions = {}
): Promise<{ baseline: ExperimentRun; treatments: ExperimentRun[] }> {
 ...
 */
export function canResumeRun(runId: string, options: { logDir?: string } = {}): boolean {
 ...
 */
export function getLastIteration(runId: string, options: { logDir?: string } = {}): number {
  const run = loadRun(runId, options);
  if (!run) {
    return 0;
  }
