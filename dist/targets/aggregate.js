 ...
 */
export function computeTargetDeltaQ(issues) {
    let totalDeltaQ = 0;
    // Group impacts by dimension to avoid double-counting
    const impactsByDimension = new Map();
...
function computeBreakdown(issues) {
    ...
    if (tsIssues.length > 0) {
        const codes = [...new Set(tsIssues.map(i => i.code).filter(Boolean))];
    ...
    if (eslintIssues.length > 0) {
        ...
        const rules = [...new Set(eslintIssues.map(i => i.code).filter(Boolean))];
        breakdown.eslint = {
