 ...
 */
export function listUntrackedCodeFiles(): readonly string[] {
  const config = getConfig();
 ...
 */
function assertTrackedCodeExists(): void {
  const config = getConfig();
  ...
  if (tracked.length === 0) {
    throw new Error(
      `No tracked files match ${config.codePathspecs.join(', ')}, so there is nothing to ` +
...
export type CodeStateDigest =
  ...
  | { readonly kind: 'no-tracked-code'; readonly message: string }
  /** Git refused the question -- usually an unknown commit. */
  | { readonly kind: 'git-failed'; readonly message: string };

export function codeStateDigest(againstCommit: string): CodeStateDigest {
  const config = getConfig();
  ...
  if (!/^(HEAD|[0-9a-f]{7,40})$/.test(againstCommit)) {
    return {
      kind: 'git-failed',
      message:
        `Refusing to ask git about ${JSON.stringify(againstCommit)}: this takes HEAD or a hex ` +
  ...
  try {
    const trackedDiff = execSync(
      `git diff ${againstCommit} ${getCodePathspec()}`,
      {
        cwd: config.projectRoot,
        encoding: 'utf-8',
        maxBuffer: GIT_MAX_BUFFER,
 ...
 */
export function digestOfUnchangedCodeState(): string {
