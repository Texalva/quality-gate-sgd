...
import { getAllDimensions, getDimensionsByCategory, formatDimensionsTable, } from './dimensions/index.js';
import { computeFitness, computeGradient, suggestNextFixes, formatFitnessScore, formatGradientTable, } from './fitness.js';
import { extractLocatedIssues, aggregateToTargets, aggregateToSymbolsWithOptions, formatTargetList, formatTargetsForJson, formatSymbolIssuesList, formatSymbolIssuesForJson, } from './targets/index.js';
import { extractSymbols, getSymbolTableStats, computeAddressFitness, formatAddressFitness, } from './symbols/index.js';
import { buildDimension, appendToConfigFile, } from './dimensions/index.js';
import { runMcpServer } from './mcp/index.js';
import { estimateFixability as runFixabilityEstimation } from './fixability/index.js';
import { createExperimentScaffold, initializeRun, executeDockerRun, listExperiments, listRuns as listDockerRuns, } from './experiments/docker/index.js';
import { analyzeBatch, generateAnalysisReport, loadBatch, visualizeBatch, visualizeResults, downloadSWEBenchSplit, checkSWEBenchLocalSplits, getSWEBenchDatasetInfo, formatBytes, progressBar, } from './experiments/index.js';
 ...
 */
function formatUnreadableSources(extracted) {
...
function formatSonarIssues(issues) {
    if (issues.length === 0)
        return '';
...
function getRemediationGuidance(failures) {
    const lines = ['\n## How to Fix\n'];
    const hasMonotonicFailure = failures.some((f) => f.type === 'monotonic');
    const hasScriptFailure = failures.some((f) => f.type === 'script');
    const hasCoverageIssue = failures.some((f) => f.rule.includes('coverage') || f.message.includes('coverage'));
    if (hasCoverageIssue) {
    ...
    if (hasMonotonicFailure && !hasCoverageIssue) {
        ...
        lines.push('');
        lines.push('A metric regressed from the baseline. Check the cache for details:');
        lines.push('```');
...
function parseRunArgs(args) {
    return {
        skipSonarQube: args.includes('--docker=no') ||
 ...
 */
function runSonarqubeScanOrExit() {
...
async function runQualityGate(options = { skipSonarQube: false }) {
    ...
    log(`Runner: ${layoutConfig.packageManager.manager} (${layoutConfig.packageManager.reason})`);
    ...
    if (existingEntry && isCacheValid(existingEntry, rules)) {
        ...
        if (existingEntry.evaluation.status === 'pass') {
            ...
            const reEvalResult = evaluateRules(rules, existingEntry.metrics, baselineEntry);
            ...
            const hasSonarFailure = reEvalResult.failedRules.some((f) => f.rule.includes('sonar') ||
    ...
    const pct = (value) => value === undefined ? 'unavailable' : `${value.toFixed(1)}%`;
    if (metrics.coverage) {
        const { lambda, unit, union } = metrics.coverage;
        if (lambda) {
            log(`  Lambda:  branches=${pct(lambda.branches)}, statements=${pct(lambda.statements)}`);
        }
        if (unit) {
            log(`  Unit:    branches=${pct(unit.branches)}, statements=${pct(unit.statements)}`);
        }
        if (union) {
            log(`  Union:   branches=${pct(union.branches)}, statements=${pct(union.statements)}`);
        }
    }
    if (metrics.sonarqube) {
        log(`  SonarQube: bugs=${metrics.sonarqube.bugs}, vulnerabilities=${metrics.sonarqube.vulnerabilities}, smells=${metrics.sonarqube.codeSmells}`);
        log(`  Severity:  blocker=${metrics.sonarqube.blocker}, critical=${metrics.sonarqube.critical}, major=${metrics.sonarqube.major}, minor=${metrics.sonarqube.minor}, info=${metrics.sonarqube.info}`);
    }
    if (metrics.typescript) {
        log(`  TypeScript: errors=${metrics.typescript.errors}`);
    }
    if (metrics.eslint) {
        log(`  ESLint: errors=${metrics.eslint.errors}, warnings=${metrics.eslint.warnings}`);
    }
    log(`  Scripts: ${Object.entries(metrics.scripts)
        .map(([k, v]) => `${k}=${v}`)
        .join(', ')}`);
    // Evaluate rules
    log('\nEvaluating rules...');
    ...
    const ungated = measurementFailures.filter((f) => !isMeasurementUnderRule(rules, f.dimension));
    ...
    if (ungated.length > 0) {
        ...
        // fix did nothing.
        log('  Not failing the gate on these -- no rule reads them, directly or through a dimension ' +
    ...
    if (result.status === 'pass') {
        ...
        // Show top SonarQube issues inline if any sonar-related failure
        const hasSonarFailure = result.failedRules.some((f) => f.rule.includes('sonar') ||
            f.rule.includes('blocker') ||
            f.rule.includes('critical') ||
            f.rule.includes('major') ||
            f.rule.includes('minor'));
 ...
 */
function runDimensionsList(args) {
    ...
    if (category) {
        ...
        const categories = ['coverage', 'errors', 'quality', 'custom'];
...
async function runSuggest(args) {
    ...
    const granularity = deepMode ? 'symbol' : 'file';
    ...
    if (quickMode) {
        ...
        if (suggestions.length === 0) {
            ...
            log(unmeasured && unmeasured.length > 0
    ...
    if (symbolsMode) {
        ...
        if (symbolIssues.length === 0) {
            log(extractedIssues.measurementFailures.length > 0
 ...
 */
async function runAddDimension(args) {
 ...
 */
async function runExperimentInit(args) {
 ...
 */
async function runExperimentInitRun(args) {
 ...
 */
async function runExperimentRun(args) {
 ...
 */
function runExperimentList(args) {
    const expArg = args.find(a => a.startsWith('--experiment='));
    const jsonFlag = args.includes('--json');
    if (expArg) {
 ...
 */
function runExperimentAnalyze(args) {
 ...
 */
function runExperimentReport(args) {
    const batchArg = args.find(a => a.startsWith('--batch='));
 ...
 */
function runExperimentVisualize(args) {
    const batchArg = args.find(a => a.startsWith('--batch='));
 ...
 */
async function runExperimentDownload(args) {
 ...
 */
async function runExperimentCommand(args) {
