...
function loadCoverageData(): {
  ...
  let unitData: CoverageSummaryJson | undefined;
  let lambdaData: CoverageSummaryJson | undefined;

  if (fs.existsSync(unitPath)) {
    try {
      unitData = JSON.parse(
        fs.readFileSync(unitPath, 'utf-8')
      ) as CoverageSummaryJson;
    } catch {
      // Skip if invalid
    }
  }
  ...
  if (fs.existsSync(lambdaPath)) {
    try {
      lambdaData = JSON.parse(
        fs.readFileSync(lambdaPath, 'utf-8')
      ) as CoverageSummaryJson;
    } catch {
      // Skip if invalid
    }
 ...
 */
function countTypescriptRootCauses(errors: TypeScriptError[]): number {
...
export function extractTypescriptMetrics(): TypescriptMetrics {
  const config = getConfig();
