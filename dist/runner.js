...
const SUPPORTED_MANAGERS = new Set(['npm', 'bun']);
...
function scriptsOf(projectRoot) {
 ...
 */
function readDeclaredManager(projectRoot) {
...
export function detectPackageManager(projectRoot) {
    ...
    if (override) {
        if (!SUPPORTED_MANAGERS.has(override)) {
            throw new Error(`${PACKAGE_MANAGER_ENV_VAR} is set to "${override}", which this tool cannot run. ` +
    ...
    const ignoredDeclaration = typeof declaredRaw === 'string' && declaredRaw.trim() !== ''
...
function resolved(executable, args) {
 ...
 */
export function scriptCommand(script, selection) {
 ...
 */
export function binaryCommand(binary, args, selection) {
 ...
 */
const WINDOWS_SHIM_SUFFIXES = ['.cmd', '.ps1', '.exe', '.bunx'];
 ...
 */
export function resolveProjectBinary(projectRoot, binary, platform = process.platform) {
    if (!existsSync(projectRoot))
        return { kind: 'root-missing', projectRoot };
    const suffixes = platform === 'win32' ? ['', ...WINDOWS_SHIM_SUFFIXES] : [''];
    const candidates = ancestorDirectories(projectRoot).flatMap((directory) => suffixes.map((suffix) => path.join(directory, 'node_modules', '.bin', `${binary}${suffix}`)));
 ...
 */
export function binaryInvocation(binary, args, selection, projectRoot) {
 ...
 */
export function installAdvice(binary, selection, projectRoot) {
    const present = (name) => existsSync(path.join(projectRoot, name));
    if (present('.pnp.cjs') || present('.pnp.js')) {
        return ('set `nodeLinker: node-modules` in .yarnrc.yml and re-run `yarn install` -- this ' +
 ...
 */
const TYPECHECK_SCRIPT_CANDIDATES = ['type-check', 'typecheck'];
/**
 ...
 */
export function manifestDefinesScript(projectRoot, script) {
 ...
 */
function definesScript(scripts, script) {
 ...
 */
export function detectTypecheckScript(projectRoot) {
    ...
    if (found) {
        const others = TYPECHECK_SCRIPT_CANDIDATES.filter((n) => n !== found && definesScript(scripts, n));
        return {
            script: found,
 ...
 */
export function readEntryManager(value) {
    if (value === undefined)
        return 'npm';
 ...
 */
export const NO_INSTALL_REFUSAL_PATTERN = /npx canceled due to missing packages|Stopping because --no-install was passed/i;
