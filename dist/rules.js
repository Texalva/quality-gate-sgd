...
function evaluateMeasurements(rules, metrics) {
    return (metrics.measurementFailures ?? [])
        ...
        message: `${failure.dimension} ${MEASUREMENT_KINDS_REPORTING_A_NUMBER.has(failure.kind)
 ...
 */
function gradesSonarqube(metricPath) {
 ...
 */
function evaluateAnalysisProvenance(rules, metrics) {
    const provenance = metrics.sonarqubeProvenance;
    if (provenance === undefined || provenance.kind === 'confirmed')
        return [];
    ...
    return [
        {
            ...
            message: `${graded.length} sonarqube rule(s) (${graded.join(', ')}) were applied to numbers ` +
        ...
        for (const metricPath of rule.metrics) {
            ...
            if (gradesSonarqube(metricPath) &&
                ...
                unevaluated.push({
                    ...
                    message: `the baseline's ${metricPath} (${baselineValue}) could not be tied to a known ` +
...
export function isCacheValid(entry, rules) {
    ...
    if (entry.metrics.sonarqube !== undefined &&
