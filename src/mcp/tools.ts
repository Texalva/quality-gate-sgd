...
export async function handleRun(args: RunArguments): Promise<{
...
export async function handleScore(args: ScoreArguments): Promise<{
  content: Array<{ type: 'text'; text: string }>;
}> {
  const skipSonarQube = args.coverageOnly ?? false;
...
export async function handleSuggest(args: SuggestArguments): Promise<{
  content: Array<{ type: 'text'; text: string }>;
}> {
  ...
  try {
    ...
    const targetGranularity: TargetGranularity = granularity === 'symbol' ? 'symbol' : 'file';
...
export async function handleTrajectory(): Promise<{
  content: Array<{ type: 'text'; text: string }>;
}> {
  try {
    const cache = loadCache();
...
export async function handleExplain(args: ExplainArguments): Promise<{
  content: Array<{ type: 'text'; text: string }>;
}> {
...
function getDirectionTips(direction: 'higher-better' | 'lower-better'): string {
