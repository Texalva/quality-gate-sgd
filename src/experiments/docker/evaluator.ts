 ...
 */
export function getImageName(instanceId: string, config: EvaluatorConfig = {}): string {
 ...
 */
function detectArch(): 'x86_64' | 'arm64' {
  const arch = os.arch();
 ...
 * Pull a Docker image.
 */
export async function pullImage(imageName: string, verbose = false): Promise<boolean> {
...
): Promise<{ available: boolean; imageName: string }> {
 ...
 */
function detectProjectType(instanceId: string): 'django' | 'pytest' {
 ...
 */
function formatDjangoTest(test: string): string {
 ...
 */
function createEvalScript(patchInfo: PatchToEvaluate): string {
  ...
  const getTestCommand = (test: string): string => {
...
async function runContainer(options: {
  ...
  return new Promise((resolve, reject) => {
    ...
    args.push(
      '-v', `${scriptFile}:/evaluate.sh:ro`,
      imageName,
      '/bin/bash', '/evaluate.sh',
    );
 ...
 */
export async function evaluatePatches(
  patches: PatchToEvaluate[],
  config: EvaluatorConfig = {},
 ...
 */
export function computeEvaluationStats(results: EvaluationResult[]): {
  total: number;
  resolved: number;
  failed: number;
  errored: number;
 ...
 * Get Docker info.
 */
export function getDockerInfo(): { version: string; available: boolean } {
  try {
