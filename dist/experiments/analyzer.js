 ...
 */
import { describe, tTest, spearmanCorrelation, chiSquaredTest, anova, linearRegression, logisticRegression, rocAuc, } from './stats.js';
// =============================================================================
// Hypothesis Descriptions
// =============================================================================
const HYPOTHESIS_DESCRIPTIONS = {
 ...
 */
function analyzeHypothesis(hypothesis, runs) {
    switch (hypothesis) {
        case 'H1':
...
function analyzeH2(runs) {
    ...
    const test = chiSquaredTest(treatmentPass, treatment.length, baselinePass, baseline.length);
 ...
 */
function analyzeH4(runs) {
    const data = runs
        .filter(r => r.outcome.passed && r.outcome.iterationsToPass !== undefined)
        .map(r => ({
        mappingCoverage: r.metadata.mappingCoverage ?? 0.5,
 ...
 */
function analyzeH5(runs) {
    const data = runs.map(r => ({
        resolution: r.metadata.callGraphResolution ?? 0.5,
 ...
 */
function analyzeH6(runs) {
    const data = runs
        .filter(r => r.outcome.passed && r.outcome.iterationsToPass !== undefined)
        .map(r => ({
        p90Sloc: r.metadata.p90AddressSloc ?? 50,
        iterations: r.outcome.iterationsToPass,
    }));
 ...
 */
export function computeWastedIterationRate(run) {
...
export function computeWastedIterationBreakdown(run) {
 ...
 */
export function generateAnalysisReport(batch, results) {
    const lines = [];
