 ...
 */
function provenanceFailureIsWarranted(
  stampOutcomes: readonly StampOutcome[],
  provenanceRequired: boolean
): (suite: CoverageSuite) => boolean {
  const codeMovedDuringRun = new Set(
    stampOutcomes
      .filter(
        (outcome) =>
  ...
  return (suite) => {
    if (!provenanceRequired) return false;
    ...
    if (codeMovedDuringRun.has(suite)) return false;
  ...
  return [
    ...verdicts.flatMap((verdict): readonly MeasurementFailure[] => {
      if (verdict.kind === 'unverifiable') {
        if (!warranted(verdict.suite)) return [];
        return [
          {
