...
import type { PatchProposalReasoning, PatchQualityMetrics, QualityGateConfig } from '../swebench/quality-gate.js';
...
/**
 * Reasoning attempt with quality evaluation.
 */
 ...
 */
export function createQualityGatedAgent(
  task: SWEBenchTask,
  config: QualityGatedAgentConfig
): ExperimentAgent {
  ...
  let reasoningAttempts: ReasoningAttempt[] = [];
  ...
  return {
    ...
    async executeIteration(
      ...
      let reasoning: PatchProposalReasoning | null = null;
      let quality: PatchQualityMetrics | null = null;
      let gateResult = null;
      ...
      for (let reasoningIter = 1; reasoningIter <= maxReasoningIterations; reasoningIter++) {
        ...
        const { reasoning: extractedReasoning, error: extractError } = await extractReasoning(
          task,
          llmConfig
        );
      ...
      try {
        ...
        const { patch: patchContent, error: patchError } = await reasoningToPatch(
          task,
          reasoning,
          config.llm
        );
