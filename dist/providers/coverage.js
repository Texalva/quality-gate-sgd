 ...
 */
function summaryIsRequired(suite, absentIsFailure, anySuiteRead) {
    if (!absentIsFailure)
        return false;
    if (suite.configured)
        return true;
 ...
 */
function unmergeableFileEntries(data) {
    if (!data)
        return [];
    const isCount = (value) => typeof value === 'number' && Number.isFinite(value);
    const mergeable = (entry) => COVERAGE_DIMENSIONS.every((dimension) => {
 ...
 */
function extractFromTotal(data, suite, evidence) {
    ...
    if (data === undefined)
        return { metrics: undefined, failures: [] };
    ...
    if (!isRecord(data.total)) {
        return fail('unparseable-output', `${evidence.command} has no \`total\` object (found ${JSON.stringify(data.total)}), so ` +
    ...
    // existed.
    const readEntry = (dimension) => total[dimension];
    ...
    const malformed = denominators.filter((d) => !isCount(d.denominator) || !isCount(d.covered));
    if (malformed.length > 0) {
        const describe = (d) => `total.${d.dimension} = {total: ${JSON.stringify(d.denominator)}, ` +
            `covered: ${JSON.stringify(d.covered)}}`;
        return fail('unparseable-output', `${evidence.command} does not report a whole non-negative count for ` +
    ...
    const inconsistent = denominators.filter((d) => d.covered > d.denominator);
    if (inconsistent.length > 0) {
        const describe = (d) => `${String(d.covered)} of ${String(d.denominator)} ${d.dimension}`;
        return fail('unparseable-output', `${evidence.command} reports more covered than total: ` +
            `${inconsistent.map(describe).join('; ')}. A report that contradicts itself cannot be ` +
            'graded -- delete it and re-run the coverage tool.');
    }
    const measured = denominators.filter((d) => d.denominator > 0);
    if (measured.length === 0) {
        const fileEntries = Object.keys(data).filter((key) => key !== 'total').length;
        return fail('measured-nothing', `${evidence.command} reports 0 statements, 0 branches, 0 functions and 0 lines across ` +
    ...
    for (const { dimension, denominator } of denominators) {
        ...
        if (typeof pct !== 'number' || !Number.isFinite(pct)) {
            return fail('unparseable-output', `${evidence.command} reports total.${dimension}.pct as ` +
    ...
    if (vacuous.length > 0) {
        ...
        console.error(`Warning: ${evidence.command} has no ${vacuous.join('/')} to measure ` +
    ...
    return { metrics: metrics, failures: [] };
}
// =============================================================================
// Issues
// =============================================================================
 ...
 */
const isReadableLocation = (value) => isRecord(value) && isRecord(value.start) && isRecord(value.end);
/**
 ...
 */
function asIstanbulFileCoverage(value) {
    if (!isRecord(value))
        return null;
    const { branchMap, fnMap, b, f } = value;
    if (!isRecord(branchMap) || !isRecord(fnMap) || !isRecord(b) || !isRecord(f))
        return null;
    for (const branch of Object.values(branchMap)) {
        ...
        if (!Array.isArray(branch.locations))
            return null;
        if (!branch.locations.every(isReadableLocation))
            return null;
    }
    for (const fn of Object.values(fnMap)) {
        if (!isRecord(fn) || !isReadableLocation(fn.loc))
            return null;
    }
    return value;
}
 ...
 * about.
 */
function asCoverageSummaryForIssues(parsed) {
    if (!isRecord(parsed))
        return null;
    for (const [filePath, entry] of Object.entries(parsed)) {
        if (filePath === 'total' || entry === null || entry === undefined)
            continue;
        if (shouldSkipCoverageFile(filePath))
            continue;
        if (!isRecord(entry))
            return null;
        for (const dimension of ['branches', 'functions']) {
            ...
            if (willFormatPct && typeof measure.pct !== 'number')
                return null;
        }
    }
    return parsed;
 ...
 */
function extractCoverageIssuesFromSummary(data, dimensionPrefix) {
    const issues = [];
...
export function createIstanbulCoverageProvider(paths, options = {}) {
    ...
    return {
        ...
        measure(context) {
            ...
            const evidenceFor = (attempt) => buildReportEvidence(`read ${attempt.path}`, elapsedMs, [attempt]);
            ...
            const readFailure = (attempt, suite, summaryRequired) => {
                ...
                if (attempt.outcome === 'absent') {
                    if (!summaryRequired)
                        return [];
                    return [
                        measurementFailure('report-missing', suite, `${attempt.path} does not exist, so no coverage was measured. A ceiling or ` +
                            ...
                            'QUALITY_COVERAGE_REQUIRED=false to say so.', evidenceFor(attempt)),
                    ];
                }
                const detail = attempt.outcome === 'invalid-json'
                ...
                return [
                    measurementFailure('unparseable-output', suite, `${attempt.path} ${detail}, so no coverage number can be derived from it. Delete it ` +
                        `and re-run the coverage tool, or point ${SUITE_SETTING[suite]} at the directory ` +
                        'that holds the real report.', evidenceFor(attempt)),
                ];
            };
            // --- metrics, from the summaries ---------------------------------------
            const summaryData = new Map(summaries.map((s) => [s.suite, s.data]));
            const perSuite = summaries.map((s) => ({
                suite: s.suite,
                ...extractFromTotal(s.data, s.suite, evidenceFor(s.attempt)),
            }));
            ...
            const unmergeableFailures = summaries.flatMap((s) => {
                const bad = unmergeableFileEntries(s.data);
                if (bad.length === 0)
                    return [];
                const named = bad.slice(0, 3).join(', ');
                const rest = bad.length > 3 ? `, and ${bad.length - 3} more` : '';
                return [
                    measurementFailure('unparseable-output', 'coverage.union', `${s.attempt.path} has ${bad.length} file entr${bad.length === 1 ? 'y' : 'ies'} ` +
                        ...
                        `point ${SUITE_SETTING[s.suite]} at the directory that holds the real report.`, evidenceFor(s.attempt)),
                ];
            });
            // --- issues, from the detail reports, falling back to the summaries -----
            const issues = [];
            const unexpectedShapes = new Set();
            ...
            for (const final of finals) {
                ...
                if (final.attempt.existed)
                    foundCoverageFinal = true;
                if (final.attempt.outcome !== 'read')
                    continue;
            ...
            if (wantIssues && issues.length === 0) {
                for (const summary of summaries) {
                    if (summary.attempt.outcome !== 'read')
                        continue;
                    ...
                    if (summaryIssues.length > 0 && foundCoverageFinal) {
                        console.error(`Warning: Using ${paths.summaryFile} fallback for ${summary.suite} coverage`);
                    }
            ...
            const metrics = {
                ...
                union: unmergeableFailures.length > 0
                    ? undefined
