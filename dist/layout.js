 ...
 */
function tracksFiles(projectRoot, pathspecs) {
 ...
 * prevented.
 */
export function assertSupportedLayout(projectRoot, codePathspecs) {
    if (!tracksFiles(projectRoot, [REQUIRED_SOURCE_DIR])) {
        throw new Error(`No tracked files under \`${REQUIRED_SOURCE_DIR}\`. This tool measures code under ` +
    ...
    if (!tracksFiles(projectRoot, codePathspecs)) {
        throw new Error(`No tracked files match QUALITY_CODE_PATHSPECS (${codePathspecs.join(', ')}), so the ` +
