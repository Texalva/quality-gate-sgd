...
async function runQualityGate(options = { skipSonarQube: false }) {
    ...
    if (metrics.coverage) {
        ...
        if (lambda) {
            log(`  Lambda:  branches=${lambda.branches.toFixed(1)}%, statements=${lambda.statements.toFixed(1)}%`);
        }
        if (unit) {
            log(`  Unit:    branches=${unit.branches.toFixed(1)}%, statements=${unit.statements.toFixed(1)}%`);
        }
        if (union) {
            log(`  Union:   branches=${union.branches.toFixed(1)}%, statements=${union.statements.toFixed(1)}%`);
        }
    }
