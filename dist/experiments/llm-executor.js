 ...
 */
import { retrieveCodeContext, formatCodeContext } from './swebench/code-retrieval.js';
 ...
 */
function buildReasoningPrompt(task, projectRoot, retrievalConfig) {
    const parts = [];
    // Task information
 ...
 */
function buildUserPrompt(task, suggestion, context) {
    const parts = [];
    // Task description
 ...
 */
async function callResponsesAPI(request, config) {
 ...
 */
async function callWithRetry(request, config) {
 ...
 */
function applyFileChanges(changes, projectRoot) {
    let applied = 0;
    const errors = [];
 ...
 */
function readFileForContext(filePath, projectRoot) {
 ...
 */
function convertToUnifiedDiff(changes, projectRoot) {
    const diffs = [];
    for (const change of changes) {
 ...
 */
export async function extractReasoning(task, config) {
 ...
 */
export async function reasoningToPatch(task, reasoning, config) {
    ...
    try {
        ...
        const changes = fixResponse.changes.map(change => {
 ...
 */
export function createLLMExecutor(config) {
    const { model = 'gpt-5-mini', projectRoot, applyChanges = true, } = config;
    return {
        async attemptFix(task, suggestion, context) {
            ...
            try {
                ...
                fixResponse = JSON.parse(jsonText);
            }
            catch {
                return {
                    attempted: true,
 ...
 */
export function createGPT5MiniExecutor(projectRoot, options = {}) {
 ...
 */
export function createGPT5NanoExecutor(projectRoot, options = {}) {
    return createLLMExecutor({
        ...options,
        projectRoot,
        model: 'gpt-5-nano',
 ...
 */
export function createGPT52Executor(projectRoot, options = {}) {
    return createLLMExecutor({
        ...options,
        projectRoot,
        model: 'gpt-5.2',
 ...
 */
export function createCustomEndpointExecutor(projectRoot, baseUrl, model, options = {}) {
    return createLLMExecutor({
        ...options,
        projectRoot,
        baseUrl,
