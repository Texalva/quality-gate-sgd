...
export interface FixabilityEstimate {
  symbolId: string;
  score: number; // 0-1: fraction of issues fixable in one pass
  reasoning: string;
 ...
 */
function extractSymbolCode(symbol: SymbolIssues): string | null {
 ...
 */
function buildPrompt(symbol: SymbolIssues, code: string): string {
  ...
  if (issues.coverage.length > 0) {
    ...
    if (branches > 0) issueList.push(`${branches} uncovered branches`);
    if (funcs > 0) issueList.push(`${funcs} uncovered functions`);
  }
 ...
 */
function isValidResponse(parsed: unknown): parsed is { score: number; effort: string; reasoning: string } {
...
async function estimateOne(
  ...
  model: string,
  maxRetries: number = 2
): Promise<FixabilityEstimate | null> {
...
Do NOT respond with an empty object. Always provide all three fields.`,
      ...
      return {
        ...
        estimatedEffort: (['trivial', 'moderate', 'significant', 'major'].includes(parsed.effort)
          ? parsed.effort
          : 'moderate') as FixabilityEstimate['estimatedEffort'],
      };
    } catch (error) {
      if (attempt < maxRetries) {
 ...
 */
export async function estimateFixability(
  symbols: SymbolIssues[],
  options: EstimateFixabilityOptions = {}
): Promise<FixabilityEstimate[]> {
  const {
    apiKey,
  ...
  if (estimates.length > 0) {
    symbols.sort((a, b) => {
      ...
      if (aEstimated && !bEstimated) return -1;
      if (!aEstimated && bEstimated) return 1;
