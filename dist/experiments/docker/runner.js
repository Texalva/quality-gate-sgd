...
import { getExperimentDirs, getRunDirs, loadRun, updateRunState, cleanWorkspace, cloneToWorkspace, } from './scaffold.js';
// =============================================================================
 ...
 */
async function dockerComposeUp(runDirs, options) {
 ...
 */
async function dockerComposeDown(runDirs) {
    return new Promise((resolve, reject) => {
 ...
 */
function watchTrajectory(runDirs, onEvent) {
    let lastSize = 0;
 ...
 */
export function parseTrajectory(trajectoryPath) {
 ...
 */
export function appendTrajectoryEvent(runDirs, event) {
    const fullEvent = {
 ...
 */
export async function executeDockerRun(experimentId, runId, options = {}) {
    const { baseDir, timeout = 30 * 60 * 1000, // 30 minutes
    cleanupOnComplete = true, followLogs = false, onStateChange, onTrajectoryEvent, onLog, } = options;
    // Load run
    ...
    try {
        ...
        const definition = JSON.parse(fs.readFileSync(experimentDirs.definition, 'utf-8'));
 ...
 */
async function waitForCompletion(composeProc, runDirs) {
    return new Promise((resolve, reject) => {
 ...
 */
function computeResult(runDirs) {
    ...
    const finalScore = lastGateQuery?.data?.qualityScore ?? 0;
    const gatePassed = lastGateQuery?.data?.passed ?? false;
    // Calculate duration
 ...
 */
export async function executeBatchRuns(runs, options = {}) {
    const { parallelism = 1, continueOnFailure = true, onRunComplete, ...runOptions } = options;
    const results = [];
